...
import com.phj.crowd.config.PayProperties;
...
public class PayHandler {
    ...
    public String generateOrder(@RequestBody OrderVO orderVO,HttpSession session) throws AlipayApiException {
        // 1.生成订单号并设置到 orderVO 对象中
        // ①根据当前日期时间生成字符串
        ...
        Double orderAmount = (double) (orderVO.getOrderProjectVO().getSupportPrice() * orderVO.getOrderProjectVO().getReturnCount() + orderVO.getOrderProjectVO().getFreight());
        orderVO.setOrderAmount(orderAmount);

        // 5.将 orderVO 放入session
        session.setAttribute("orderVO", orderVO);

        // 6.调用专门封装好的方法给支付宝接口发送请求
    ...
    private String sendRequestToAliPay(String outTradeNo, Double totalAmount, String subject, String body) throws AlipayApiException {
        ...
        alipayRequest.setBizContent("{\"out_trade_no\":\"" + outTradeNo + "\","
                ...
                + subject + "\"," + "\"body\":\"" + body + "\"," + "\"product_code\":\"FAST_INSTANT_TRADE_PAY\"}");
    ...
    public String returnUrlMethod(HttpServletRequest request, HttpSession session) throws AlipayApiException, UnsupportedEncodingException {
        ...
        boolean signVerified = AlipaySignature.rsaCheckV1(params,
                payProperties.getAlipayPublicKey(), payProperties.getCharset(),
    ...
    public void notifyUrlMethod(HttpServletRequest request) throws UnsupportedEncodingException, AlipayApiException {
        ...
        实际验证过程建议商户务必添加以下校验：
             ...
             3、校验通知中的 seller_id（或者 seller_email) 是否为 out_trade_no 这笔单据的对应的 操作方
